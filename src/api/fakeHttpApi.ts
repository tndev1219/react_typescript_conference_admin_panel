...
let callForPapersData: CallForPapersData = {
    byTalkId: {
...
const fakeResponse = <D>(status: number, data?: D): Promise<[number, D | undefined]> => {
...
const setup =
    (): void => {
        ...
        httpMock
            .onGet("/talks")
            .reply(() => {
                const basicTalksData: BasicTalkData[] = talksData.map(talk => ({
                    id: talk.id,
                    title: talk.title,
        ...
        httpMock
            .onPut(/^\/callForPapers\/.+/)
            .reply((config: { url?: string, data?: string }) => {
